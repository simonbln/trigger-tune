...
# Original code by Stewart Watkiss @PenguinTutor
# https://www.penguintutor.com
# Source: https://github.com/penguintutor/dfplayermini-pico
# License: GNU General Public License v3.0 (GPL-3.0)
#
# Modified by simonbln - March 2026
# Changes:
# - command.to_bytes(1) -> command.to_bytes(1, 'big') for compatibility with newer Python versions
# - Fixed reset command return value parsing
...
class DFPlayerMini ():
    ...
    def send_command (self, command, value=0, feedback=1):
        ...
        data = b'\xFF\x06' + command.to_bytes(1, 'big') + feedback.to_bytes(1, 'big') + value.to_bytes(2, 'big')
    ...
    def reset(self):
        ...
        #print(return_value)
        #print(len(return_value))
        if len(return_value) == 1 and return_value[0] ==0x00:
            return True
        if return_value == None or len(return_value) != 10:
            # If so just return without checking for a value
            return False
